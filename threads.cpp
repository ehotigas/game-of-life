/*participantes:
        Beatriz Assumção da Silva - 149418
...
public:
  ...
  Grid *grid;
  Grid *newGrid;
  GridThread(int startIndex, int endIndex, Grid *grid, Grid *newGrid)
      : startIndex(startIndex), endIndex(endIndex), grid(grid),
        newGrid(newGrid) {}
  ~GridThread() {
    delete &startIndex;
    delete &endIndex;
  ...
  void start() { pthread_create(&ptid, 0, &run, this); }

  void join() { pthread_join(ptid, NULL); }

  void exit() { pthread_join(ptid, NULL); }
...
public:
  ...
  Grid *getNewGrid(int numThreads) {
    ...
    GridThread **threads = (GridThread **) malloc(numThreads * sizeof(GridThread *));
    int lines = ((int)N) / numThreads + 1;
    for (int countThread = 0; countThread < numThreads; countThread++) {
      int start = lines * countThread;
      int end = lines * countThread + lines <= N ? lines * countThread + lines : N;
      threads[countThread] = new GridThread(start, end, this, newGrid);
      threads[countThread]->start();
    }

    for (int countThread = 0; countThread < numThreads; countThread++) {
      threads[countThread]->join();
      threads[countThread]->exit();
      // cout << "exited" << countThread << endl;
    }
    // for (int countThread = 0; countThread < numThreads; countThread++) {
    //   free(threads[countThread]);
    // }
    // free(threads);
    return newGrid;
  }
...
void *run(void *args) {
  ...
  GridThread *_this = static_cast<GridThread *>(args);
  for (int lineIndex = _this->startIndex; lineIndex < _this->endIndex;
       lineIndex++) {
    for (int columnIndex = 0; columnIndex < _this->grid->N; columnIndex++) {
      _this->newGrid->grid[lineIndex][columnIndex] =
          _this->grid->getNextCellState(lineIndex, columnIndex);
    }
  }
  pthread_exit(NULL);
}
...
Grid *play(int generations, int numThreads, Grid *grid) {
  int i;
  Grid *newGrid, *_grid=grid->getNewGrid(numThreads);
  cout << "Geração: 1 - " << _grid->countRemainingLivingCells() << endl;
  _grid->print();
  for (i = 0; i < generations - 1; i++) {
    newGrid = _grid->getNewGrid(numThreads);
    newGrid->print();
    // delete _grid;
    _grid = newGrid;
...
int main() {
  clock_t t;
  t = clock();
  int numThreads = 8, generations = 5, N = 2048;
  ...
  grid = play(generations, numThreads, grid);
  printf("\nthere are still {%i} living cells\n", grid->countRemainingLivingCells());
  t = clock() - t;
  double time_taken = ((double)t) / CLOCKS_PER_SEC;
  printf(" took %f seconds.\n", time_taken);
}
